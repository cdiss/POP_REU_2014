...
int main(int argc, char* argv[]) {
    ...
    double horizsize = size*16.0/9.0;

    wkf_timerhandle timer = wkf_timer_create();


    unsigned maxIters = 20050;
    double startReal = centerReal - (horizsize/2.0), startImag = centerImag + (size/2.0);
    double stepsize = size/steps;
    float time;
 
    if (bench) {
        int warmUpRuns = 10;
        for (int i = 0; i < warmUpRuns; i++) {
            serialFloat(0.0f, 0.0f, steps, horizsteps, 0.004, output, 500);
        }
    }
